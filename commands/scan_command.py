...
class ScanCommand(BaseCommand):
    ...
    def configure_parser(parser: argparse.ArgumentParser):
        parser.add_argument('interface', nargs='?',
                            help='Interface (Managed or Monitor mode OK)',
                            metavar='INTERFACE')
        parser.add_argument('-b', '--bssid',
                            help='Target specific BSSID',
                            metavar='BSSID')
        parser.add_argument('-l', '--loops', type=int, default=1,
                            help='number of scan loops to perform',
                            metavar='LOOPS')
        parser.add_argument('-n', '--no-stop', action='store_true',
                            help='scan indefinitely until stopping')
        parser.add_argument('-r', '--reverse', action='store_true',
                            help='reverse output')
        parser.add_argument('-o', '--out',
                            help='specify file to save results (default results.json)',
                            metavar='OUTPUT')
